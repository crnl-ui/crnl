...
Eleven pages showing **every class and every token the system ships** — 751 classes and
182 tokens — rendered live with the real CSS. Not a mockup of the system; the
...
| `06-cards.html` | Closed and open cards, three grid rhythms, logo blocks, matchups, the tile and its interactivity rule |
...
every type a text class. Team names, opponents, crests and photographs come
through `ContentLoader` from `content/` (`RULES §6`), so the whole set re-themes
with the team switcher rather than showing one team's screenshots.
...
- `sheet.js` — cross-sheet navigation from one manifest, live token resolution,
  and the loader wiring for the photographic slots.
