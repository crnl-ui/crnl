...
**Why all 369 are committed as `.ttf`, and what trimming would save.** These are
the desktop and Figma install set — Font Book and Figma take `.ttf`/`.otf`, not
`.woff`/`.woff2`, so converting them would break the only thing this directory
is for. The web set is a separate build: `fonts/*.woff2`, 145 files, 8.4 MB,
regenerated by `npm run build:fonts`.

Format is also the wrong lever, because git already deflates every blob and
`.ttf` is uncompressed binary while `.woff`/`.woff2` are not:

| | files | on disk | in git |
|---|---|---|---|
| `.ttf` (as committed) | 369 | 64.6 MB | ~29.9 MB |
| same cuts as `.woff` | 369 | 29.8 MB | ~29.6 MB |
| same cuts as `.woff2` | 369 | 21.6 MB | ~21.5 MB |

So `.woff` would halve the checkout and save 1% of the repo. Count is the lever
that works:

| kept | files | on disk | in git |
|---|---|---|---|
| shipping default per family | 145 | 25.3 MB | ~11.6 MB |
| all roman, both cases | 256 | 45.8 MB | ~21.1 MB |
| everything (current) | 369 | 64.6 MB | ~29.9 MB |

Dropping to the shipping default cut per family — what `install-fonts.sh` installs
with no argument — saves ~18 MB in the repo and keeps desktop installation
working. What goes is the 113 italics and the non-default case variant of each
family. Both are regenerable: `build_fonts.py` and `build_italics.py` re-cut them
from `google/fonts` sources.
